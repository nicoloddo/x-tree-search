...
class AlphaBetaExplainer:
    ...
    def _get_adjectives():
        ...
        adjectives = [
            BooleanAdjective(
                "final move", definition="node.final_node"
            ),  # node is_leaf and not max_search_depth_reached
            BooleanAdjective(
                ...
                explanation=ConditionalExplanation(
                    ...
                    explanation_if_true=ConditionalExplanation(
                        ...
                        explanation_if_false=ConditionalExplanation(
                            condition=If(
                                "possession", "a loss"
                            ),  # It's not a win, is it a loss or a draw?
                    ...
                    explanation_if_false=ConditionalExplanation(
                        ...
                        explanation_if_false=ConditionalExplanation(
                            ...
                            explanation_if_false=Possession(
                                "as future position after few moves"
                            ),
                            # The node has been pruned, we only have one children future consequence
                            explanation_if_true=ConditionalExplanation(
                                condition=If(
                                    "possession", "as next move", "final move"
                                ),  # Is the next move (the only one available) a final move?
                                # The only children of the move that we considered is actually a final move.
                                # In this case, we don't need to explain why the node was not in possession of futures worth exploring after checking the first possible next move,
                                # users usually understand easily why this next possible move is not in possession of futures worth exploring after checking the first possible next move when it is a final move.
                                ...
                                # Next move is not a final move, thus we need explain that the node was simply not in possession of futures worth exploring after checking the first possible next move and why.
            ...
            BooleanAdjective(
                ...
                explanation=ConditionalExplanation(
                    ...
                    explanation_if_true=CompositeExplanation(
                        Assumption(
                            "The opponent can choose to do this possible next move, or something even worse for me."
                        ),
                        ConditionalExplanation(
                            condition=If(
                                "comparison",
                            ...
                            explanation_if_true=Comparison(
                                "as next possible move",
                                "already worse for me than the alternative coming after",
                                "upperbound",
                                forward_possessions_explanations=False,
                            ),  # Do not forward explanations for "as next possible move" and "upperbound"
                            explanation_if_false=Comparison(
                                "as next possible move",
                                "already equal to the alternative coming after",
                                "upperbound",
                                forward_possessions_explanations=False,
                            ),
                        ),
                    ...
                    explanation_if_false=CompositeExplanation(
                        ...
                        Assumption(
                            "We could choose to do this move, or something even worse for the opponent."
                        ),
                        ConditionalExplanation(
                            condition=If(
                                "comparison",
                            ...
                            explanation_if_true=Comparison(
                                "as next possible move",
                                "already better for me than the alternative coming after",
                                "lowerbound",
                                forward_possessions_explanations=False,
                            ),  # Do not forward explanations for "as next possible move" and "lowerbound"
                            explanation_if_false=Comparison(
                                "as next possible move",
                                "already equal to the alternative coming after",
                                "lowerbound",
            ...
            PointerAdjective(
                ...
                explanation=ConditionalExplanation(
                    ...
                    explanation_if_false=ConditionalExplanation(
                        ...
                        ),  # The next move is the best the opponent can do
                        explanation_if_false=CompositeExplanation(
                            Assumption("On our turn we take the maximum rated move."),
                            Possession("as next move", "the best for me"),
                        ),
                    ),
                    # It was not in possession of futures worth exploring after checking the first possible next move, we just took the first move
            ...
            PointerAdjective(
                ...
                explanation=CompositeExplanation(
                    Assumption(
                        "We assume the opponent will do their best move and us our best move.",
                        necessary=True,
                    ),
                    RecursivePossession(
                        "as next move",
                        any_stop_conditions=[
                            If("possession", "as next move", "a win"),  # final move
            ...
            PointerAdjective("lowerbound", definition="node.parent.beta"),
            PointerAdjective("upperbound", definition="node.parent.alpha"),
            ComparisonAdjective(
                "better for me than",
                "score",
                ">",
                explain_with_adj_if=(
                    If("comparison", "equal to", COMPARISON_AUXILIARY_ADJECTIVE),
                    "equal to",
                ),
            ),
            ComparisonAdjective(
            ...
            ComparisonAdjective(
                "already better for me than the alternative coming after", "score", ">"
            ),
            ComparisonAdjective(
                "already worse for me than the alternative coming after", "score", "<"
            ),
            ComparisonAdjective(
                "already equal to the alternative coming after", "score", "=="
            ),
            NodesGroupPointerAdjective(
            ...
            MaxRankAdjective(
                "the best the opponent can do",
                ["worse for me than", "at least equal to"],
                "possible alternative moves",
                explain_with_adj_if=(
                    If("possession", "opponent player turn"),
                    "the best for me",
