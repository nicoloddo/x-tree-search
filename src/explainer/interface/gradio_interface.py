...
adjective_classes = {
    ...
    for name, cls in inspect.getmembers(
        sys.modules["src.explainer.adjective"], inspect.isclass
    )
...
explanation_classes = {
    name: cls
    for name, cls in inspect.getmembers(
        sys.modules["src.explainer.explanation"], inspect.isclass
    )
...
class ExplainerGradioInterface:
    ...
    def __init__(self, *, explainer=None, explain_in_hyperlink_mode=True):
        ...
        self.framework = explainer.framework or ArgumentationFramework(
            refer_to_nodes_as="node"
        )
        self.load_framework()
        self.explain_in_hyperlink_mode = explain_in_hyperlink_mode

        self.graph_visualizer = self.GraphVisualizer(
            self.nodes, depth=3, get_adjective_names=self.get_adjective_names
        )
        self.ai_explainer = self.AIExplainer(
            self.init_explainer, explain_in_hyperlink_mode
        )
...
"""
    ...
    class InterfaceBuilder:
        ...
        def __init__(self, **kwargs):
            ...
            self.check_if_comparison_adjective = kwargs.get(
                "check_if_comparison_adjective"
            )
        ...
        def update_dropdowns(self):
            ...
            return [
                ...
                for _ in range(len(self.dropdown_components))
            ]
        ...
        def get_decision_tree_legend(self, game):
            ...
            """
            return (
                game.explaining_agent.core.visualize_legend_move_tree()
        ...
        def build_ai_explanation_components(
            self,
            game_state_component,
            explainer_state_component,
            toggles: Dict[str, tuple[str, bool]] = None,
            *,
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    components["id_input"] = gr.Textbox(
                        label="Ask further about (you can drag and drop a move from the explanation)",
                        visible=self.explain_in_hyperlink_mode,
                    )
                with gr.Column(scale=1):
                    components["explain_adj_name"] = gr.Dropdown(
                        choices=self.get_adjective_names(),
                        label="Why is it ...? Why it has that ...?",
                        visible=self.explain_in_hyperlink_mode,
                    )
            components["comparison_id_input"] = gr.Textbox(
                label="In comparison to (you can drag and drop from the explanation)",
                visible=False,
            )

            # Hidden components for storing current state
            components["current_node_id"] = gr.Textbox(
                label="Current Node ID", visible=False
            )
            components["current_adjective"] = gr.Textbox(
                label="Current Adjective", visible=False
            )
            components["current_comparison_id"] = gr.Textbox(
                label="Current Comparison ID", visible=False
            )

            # Explanation button and depth slider
            components["explain_button"] = gr.Button(
                "Explain", visible=self.explain_in_hyperlink_mode
            )
            default_explanation_depth = (
                self.init_explainer.settings.explanation_depth
                if self.init_explainer is not None
                else ExplanationSettings.default_settings["explanation_depth"]
            )
            components["explanation_depth"] = gr.Slider(
                minimum=min(EXPLANATION_DEPTH_ALLOWED_VALUES),
                maximum=max(EXPLANATION_DEPTH_ALLOWED_VALUES),
                step=1,
                value=default_explanation_depth,
                label="Explanation Depth",
            )

            # Explanation output components
            components["explaining_question"] = gr.HTML(
                value=ExplainerGradioInterface.cool_html_text_container.format(
                    "Why ...?"
                ),
                label="Question",
                visible=self.explain_in_hyperlink_mode,
            ...
            if self.explain_in_hyperlink_mode and additional_info is not None:
                components["additional_info"] = gr.HTML(
                    value=ExplainerGradioInterface.cool_html_text_container.format(
                        additional_info
                    )
                )
        ...
        def build_visualize_components(self):
            ...
            components = {}
            gr.Markdown(
                """# Visualize the chain of explanations defined in the framework
            ...
            components["root_adjective"] = gr.Dropdown(
                choices=self.get_adjective_names(),
                label="Select Root Adjective (optional)",
            )
        ...
        def build_visualize_decision_tree_components(self, game_state_component):
            ...
            components["visualize_decision_tree_button"] = gr.Button(
        ...
        def build_explainer_settings_components(self, game_state_component):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    components["print_depth"] = gr.Checkbox(
                        label="Print Depth", value=default_values["print_depth"]
                    )
                    components["print_implicit_assumptions"] = gr.Checkbox(
                        label="Print Implicit Assumptions",
                        value=default_values["print_implicit_assumptions"],
                    )
            ...
            components["settings_confirmation"] = gr.Markdown("", visible=True)

            return components

        def connect_components(
            self, components, game_state_component, explainer_state_component
        ):
            """
            ...
            if "explain_button" in components:
                components["explain_button"].click(
                    ...
                    inputs=[
                        ...
                        explainer_state_component,
                        components["id_input"],
                        components["explain_adj_name"],
                        components["comparison_id_input"],
                    ...
                    outputs=[
                        ...
                        components["current_comparison_id"],
                    ],
                )
            ...
            # Update dropdowns
            self.dropdown_components = [
                comp
                for comp in [
                    components.get("root_adjective"),
                    components.get("explain_adj_name"),
                ]
                if comp is not None
            ]
        ...
        def update_ai_explanation(
            ...
            if adjective is None:
                ...
                if node is None:
                    ...
                    if self.explain_in_hyperlink_mode:
                        activated = LogicalExpression.set_hyperlink_mode(
                            self.explain_in_hyperlink_mode, node.__class__
                        )
                    else:
                        activated = False
                    ...
                    if comparison_id is None:
                        ...
                        comparison_node = game.explaining_agent.core.nodes[
                            comparison_id
                        ]
                        explanation = explainer.explain(
                            node,
                    ...
                    if isinstance(explanation, Implies):
                        ...
                        if (
                            object_str.count(
                                ExplainerGradioInterface.node_hyperlink_prefix
                            )
                            == 1
                        ):  # We are simply comparing two nodes
    ...
    class GraphVisualizer:
        ...
        def _process_node(
            ...
            is_duplicate, node_id = self._handle_duplicate_node(
                node_id, processed_nodes
            )
            ...
            if (
                ...
                and (not is_duplicate or is_root)
                and node.explanation
            ):
                self._process_explanation(
                    dot,
                    node_id,
                    node.explanation,
                    f"{node_id}_exp",
        ...
        def _process_explanation(
            ...
            if isinstance(explanation, Assumption):
                ...
                self._process_conditional_explanation(
                    dot,
                    parent_id,
                    explanation,
                    exp_id,
                ...
                self._process_generic_explanation(
                    dot,
                    parent_id,
                    explanation,
                    exp_id,
        ...
        def _process_conditional_explanation(
            self,
            dot,
            parent_id,
            explanation,
            ...
            self._process_explanation(
                dot,
                condition_id,
                explanation.explanation_if_true,
                f"{exp_id}_true",
                processed_nodes,
                explanation_depth,
                "True",
            ...
            self._process_explanation(
                dot,
                condition_id,
                explanation.explanation_if_false,
                f"{exp_id}_false",
                processed_nodes,
                explanation_depth,
                "False",
        ...
        def _process_generic_explanation(
            self,
            dot,
            parent_id,
            explanation,
            ...
            for key, value in explanation.__dict__.items():
                if (
                    key != "explanation_of_adjective"
                    and isinstance(value, str)
                    and value in self.get_adjective_names()
                ):
                    self._process_node(
                        dot,
                        value,
