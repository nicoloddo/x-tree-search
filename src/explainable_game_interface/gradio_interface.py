...
class ExplainableGameGradioInterface(GameGradioInterface):
    ...
    def __init__(
        ...
        game,
        game_title_md,
        action_spaces_to_visualize,
        explainer=None,
        interface_hyperlink_mode=True,
        game_explanation_ratio="1:2",
        *,
        help_md=None,
    ):
        """
        ...
        if not hasattr(game, "explaining_agent"):
            raise AttributeError(
                "The game instance does not have an 'explaining_agent' attribute, thus it does not support the gradio interface."
            )
        if not hasattr(game, "get_current_player"):
            raise AttributeError(
                "The game instance does not have a 'get_current_player' method, thus it does not support the gradio interface."
            )
        super().__init__(
            game,
        ...
        self.explainer_interface = ExplainerGradioInterface(
            explainer=self.explainer, explain_in_hyperlink_mode=interface_hyperlink_mode
        )
    ...
    def create_interface(self):
        ...
        with gr.Blocks(
            ...
            with gr.Tabs():
                with gr.TabItem(
                    "Game and Explain", id=self.explainer_interface.tab_ids["other"] + 0
                ):
                    with gr.Row(equal_height=False):
                        with gr.Column(scale=self.game_scale):
                            game_components = super().build_interface()
                            main_board_gallery = game_components["board_gallery"][
                                self.main_action_space_id
                            ]
                        ...
                        with gr.Column(scale=self.explanation_scale):
                            ...
                            self.ai_explanation_components = self.explainer_interface.interface_builder.build_ai_explanation_components(
                                ...
                                toggles={
                                    "skip_score_toggle": (
                                        "Skip Score Statement (the explainer is designed around skipping the score statement, problems may arise when this is disabled)",
                                        True,
                                    )
                                },
                ...
                with gr.TabItem(
                    ...
                    self.explainer_interface.tab_ids["visualize_decision_tree"],
                ):
                ...
                with gr.TabItem(
                    "Visualize Framework", self.explainer_interface.tab_ids["visualize"]
                ):
                    visualize_components = (
                        self.explainer_interface.interface_builder.build_visualize_components()
                    )

            # *********************************
            # 2. Handle components connections
            #
            self.explainer_interface.interface_builder.connect_components(
                {
                    **self.ai_explanation_components,
                    **visualize_components,
                    **visualize_decision_tree_components,
            ...
            def toggle_skip_score(
                game,
                explainer,
                skip_score: bool,
                current_node_id: str,
                current_adjective: str,
                current_comparison_id: str,
                explanation_depth: int,
            ):
                """
                Toggle the skip score statement setting and update the explanation.
                """
                explainer.frameworks["highlevel"].get_adjective(
                    "score"
    ...
    def update(self, game, explainer, show_node_id=None):
        ...
        if game is None:
            return (
                gr.Gallery(),
                "Start a Game to show the board",
                None,
                None,
                None,
                None,
        ...
        board_gallery, showing_state, show_node_id = self.update_board_gallery(
            game, show_node_id
        )
        status = self.get_updated_status(game)
        ai_explanation = self.ai_explanation_components["explanation_output"].value
        explaining_question = self.ai_explanation_components[
            "explaining_question"
        ].value
    ...
    def update_board_gallery(self, game, node_id=None):
        ...
        if node_id is not None:
            ...
            if game.explaining_agent is not None:
                node_id = (
                    game.explaining_agent.choice.id
                    if game.explaining_agent.choice
                    else ""
                )
                parent_state = (
                    game.explaining_agent.choice.parent_state
                    if game.explaining_agent.choice
                    else None
                )
            else:
        ...
        for i in range(board_state.shape[0]):
            for j in range(board_state.shape[1]):
                ...
                is_changed = (
                    parent_state is not None and parent_state[i, j] != cell_value
                )
    ...
    def update_showing_state(self, game, showing_state):
        ...
        if showing_state is not None:
            showing_state = self.explainer_interface.transform_hyperlink_to_node_id(
                showing_state
            )

        if (
            showing_state is not None
            and showing_state in game.explaining_agent.core.nodes
        ):
    ...
    def get_updated_status(self, game):
        ...
        if game.ended:
            ...
            if winner is None:
                ...
                return f"Game Over! Player {game.agents[winner, 0]} wins!"
        else:
            next_player = game.agents[game.get_current_player().id, 0]
